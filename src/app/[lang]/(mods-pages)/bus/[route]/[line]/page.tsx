...
import {addMinutes, formatDate, isSameMinute, subMinutes, isWeekend, subHours, format, endOfMinute, startOfMinute, differenceInMinutes} from 'date-fns';
...
const LineDisplayPage = () => {
    ...
    const busOfInterest = useMemo(() => {
        ...
        const busses =  [...UphillBuses.map(m => ({ ...m, direction: 'up' })), ...DownhillBuses.map(m => ({ ...m, direction: 'down' }))].filter(bus => getTimeOnDate(time, bus.time) > time30minAgo);
        // then according to the line, filter out the busses that are not on the line
        if (line == 'green') 
            return busses.filter(bus => bus.route == '校園公車' && bus.line == 'green')
                        .filter(bus => {
                            // remove duplicate down busses
                            if(bus.direction == 'down') {
                                const upBus = busses.find(b => b.direction == 'up' && isSameMinute(getTimeOnDate(time ,b.time), subMinutes(getTimeOnDate(time, bus.time), 7)));
                        ...
                        .map(bus => {
                            ...
                            const stationIndex = lineData.stations_zh.findIndex((station, i) => i >= startIndex && addMinutes(startTime, lineData.timings.slice(0, i - startIndex).reduce((a, b) => a + b, 0)) >= startOfMinute(time));
                            const stationDepTime = addMinutes(startTime, lineData.timings.slice(0, stationIndex - startIndex).reduce((a, b) => a + b, 0));
                            return { ...bus, stationIndex, stationDepTime }
                        })
                        .filter(bus => bus.stationIndex != -1);
        if (line == 'red') return busses.filter(bus => bus.route == '校園公車' && bus.line == 'red')
                        .filter(bus => {
                            // remove duplicate down busses
                            if(bus.direction == 'down') {
                                const upBus = busses.find(b => b.direction == 'up' && isSameMinute(getTimeOnDate(time ,b.time), subMinutes(getTimeOnDate(time, bus.time), 7)));
                        ...
                        .filter(bus => bus.stationIndex != -1);
        if (line == 'nanda_up') return busses.filter(bus => bus.route == '南大區間車' && bus.direction == 'up')
                        .map(bus => {
                            const startIndex = 0
                            const startTime = getTimeOnDate(time, bus.time);
                            const stationIndex = lineData.stations_zh.findIndex((station, i) => i >= startIndex && addMinutes(startTime, lineData.timings.slice(0, i - startIndex).reduce((a, b) => a + b, 0)) >= startOfMinute(time));
                        ...
                        .filter(bus => bus.stationIndex != -1);
        if (line == 'nanda_down') return busses.filter(bus => bus.route == '南大區間車' && bus.direction == 'down')
                        .map(bus => {
                            const startIndex = 0
                            const startTime = getTimeOnDate(time, bus.time);
                            const stationIndex = lineData.stations_zh.findIndex((station, i) => i >= startIndex && addMinutes(startTime, lineData.timings.slice(0, i - startIndex).reduce((a, b) => a + b, 0)) >= startOfMinute(time));
        ...
        return (language == 'zh' ?lineData.stations_zh : lineData.stations_en).map((station, i) => {
            ...
            if (!bus) {
                ...
                if (nextBus) {
                    ...
                    const stationDepTime = addMinutes(startTime, lineData.timings.slice(0, i - nextBus.stationIndex).reduce((a, b) => a + b, 0));
                    const diffMin = differenceInMinutes(stationDepTime, time);
                    return { state: BusStationState.UNAVAILABLE, station, time: diffMin < 8 ? `${diffMin} min`: formatDate(stationDepTime, 'HH:mm'), bus: bus };
                }
            }
            else {
                if (bus.stationIndex == i) {
                    ...
                    if (isSameMinute(time, bus.stationDepTime)) return { state: BusStationState.AT_STATION, station, time: dict.bus.departing, bus: bus };
                    if (time < bus.stationDepTime) return { state: BusStationState.ARRIVING, station, time: diffMin < 8 ? `${diffMin} min`: formatDate(bus.stationDepTime, 'HH:mm'), bus: bus };
                    if (time > bus.stationDepTime) return { state: BusStationState.LEFT, station, time: format(bus.stationDepTime, 'HH:mm'), bus: bus };
                }
            }
            return { state: BusStationState.UNAVAILABLE, station, time: bus?.time ?? '', bus };
        });
    }, [time, busOfInterest]);
