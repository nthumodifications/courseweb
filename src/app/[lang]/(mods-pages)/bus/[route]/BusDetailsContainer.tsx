...
const BusDetailsContainer = ({
  ...
  const [weektab, setWeektab] = useState<"weekday" | "weekend">(
  ...
  const renderBusSchedule = (
    busUp: BusDepartureDetails[],
    busDown: BusDepartureDetails[],
  ) => {
    //only display busses after selected hour
    const transformedBusesUp = busUp
      .filter((b) => getTimeOnDate(new Date(), b.time) >= selectedHour)
      .map((m) => exportNotes(m, lang));
    const transformedBusesDown = busDown
      .filter((b) => getTimeOnDate(new Date(), b.time) >= selectedHour)
      .map((m) => exportNotes(m, lang));
  ...
  const scrollToClosestTime = (now: Date) => {
    ...
    if (
      ...
      getTimeOnDate(now, lastBusDown.time) < now
    ) {
      return;
    }
    const mergedBuses = [...busesUp, ...busesDown].sort(
      (a, b) =>
        getTimeOnDate(now, a.time).getTime() -
        getTimeOnDate(now, b.time).getTime(),
    );
    ...
    setTimeout(() => {
      const closestTimeElement = document.querySelector(
        `[data-time="${closestTime.time}"]`,
      );
  ...
  const filteredHoursDate = hoursDate.filter((h) =>
    up[weektab].some(
      (b) =>
        isSameHour(getTimeOnDate(h, b.time), h) ||
        down[weektab].some((b) => isSameHour(getTimeOnDate(h, b.time), h)),
    ),
  );
  ...
  useEffect(() => {
    if (weektab == "weekend") {
      const minTime = Math.min(
        ...up.weekend.map((b) => getTimeOnDate(new Date(), b.time).getTime()),
        ...down.weekend.map((b) => getTimeOnDate(new Date(), b.time).getTime()),
      );
      setSelectedHour(startOfHour(new Date(minTime)));
    } else {
      const minTime = Math.min(
        ...up.weekday.map((b) => getTimeOnDate(new Date(), b.time).getTime()),
        ...down.weekday.map((b) => getTimeOnDate(new Date(), b.time).getTime()),
      );
      setSelectedHour(startOfHour(new Date(minTime)));
    }
      ...
      >
        <div className="w-full flex flex-col gap-4 sticky -top-8 pt-4 z-10 bg-background">
          ...
          <TabsList className="w-full">
            ...
            <TabsTrigger className="flex-1" value="weekend">
              {dict.bus.weekends}
            </TabsTrigger>
          </TabsList>
          ...
          <Table className="border border-border table-fixed">
            <TableHeader>
              <TableRow>
                <TableCell className="font-semibold text-center border border-border py-2 w-1/2">
                  {up.title}
                </TableCell>
                <TableCell className="font-semibold text-center border border-border py-2 w-1/2">
                  {down.title}
