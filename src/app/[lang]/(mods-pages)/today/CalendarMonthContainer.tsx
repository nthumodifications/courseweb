...
export const CalendarMonthContainer = ({ displayMonth, onChangeView }: { displayMonth: Date[]; onChangeView: (view: 'week', date: Date) => void }) => {
    const { events } = useCalendar();
    const isScreenMD = useMediaQuery('(min-width: 768px)');
    const rows_length = Math.ceil(displayMonth.length / 7);

    const renderEventsInDay = useCallback((day: Date, padding: number) => {
        ...
        return (
            <div className="flex flex-col gap-0.5 mt-1" key={day.getTime()}>
                {Array(padding).fill(0).map((_, index) => (<div className="w-full" style={{ height: isScreenMD ? 20: 16 }}></div>))}
    ...
    const renderAllDayEvents = useCallback((start: Date, end: Date) => {
        const allDayEvents = eventsToDisplay(events, startOfDay(start), endOfDay(end))
            .filter(e => e.allDay)
            .map(event => {
                // Snap the event to the start if it starts before the start of the week
                const snippetStart = isSameWeek(start, event.start) ? event.start : startOfDay(start);
                // Snap the event to the end if it ends after the end of the week
                const snippetEnd = isSameWeek(end, event.end) ? event.end : endOfDay(end);
                // Determine the text color
                const brightness = getBrightness(event.color);
                // From the brightness, using the adjustBrightness function, create a complementary color that is legible
                const textColor = adjustLuminance(event.color, brightness > 186 ? 0.2 : 0.95);
                const span = Math.min(differenceInDays(endOfDay(snippetEnd), startOfDay(snippetStart)) + 1, 7 - getDay(snippetStart) + 1);
        ...
        return allDayEvents.map((event, index) => (
            <EventPopover key={index} event={event}>
                <div
                    style={{ position: 'absolute', top: 33 + index * (isScreenMD ? 22: 18) , left: `calc(${event.left}% + 4px)`, width: `calc(${event.width}% - 8px)` }}
                >
                    <div
                        className="bg-nthu-500 rounded-md p-0.5 md:p-1 flex flex-row gap-1 items-center hover:shadow-md cursor-pointer transition-shadow select-none"
                        style={{ background: event.color, color: event.textColor }}
    ...
    const renderRow = useCallback((day: Date, colIndex: number) => {
        //check for any allday events that exists in the day
        const allDayEvents = events.filter(e => e.allDay && (isSameWeek(day, e.start) || isSameWeek(day, e.end)));
            ...
            <div className="flex flex-col md:min-w-0 h-full">
                <div className="grid grid-cols-7 gap-4">
                    {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day, index) => (
                        <div key={index} className="text-slate-500 text-sm font-semibold text-center">
                            {day}
                        </div>
                    ))}
                </div>
                <Separator orientation="horizontal" />
                <div className="grid flex-1" style={{ gridTemplateRows: `repeat(${rows_length}, 1fr)` }}>
                    {Array.from({ length: rows_length }).map((_, rowIndex) => (
                        <div key={rowIndex} className="grid grid-cols-7 relative">
                            {displayMonth.slice(rowIndex * 7, rowIndex * 7 + 7).map((day, colIndex) => renderRow(day, colIndex))}
                        </div>
                    ))}
                </div>
            </div>
