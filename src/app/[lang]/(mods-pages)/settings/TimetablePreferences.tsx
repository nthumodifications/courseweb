...
const TimetablePreferences: React.FC<SettingsControlProps> = ({
  ...
  const handleDisplayChange = (key: "title" | "code" | "time" | "venue") => {
    onSettingsChange({
      ...settings,
    ...
    <div className="flex flex-col gap-2">
          ...
          <Select value={settings.align} onValueChange={handleAlignChange}>
            ...
            <SelectContent>
              <SelectItem value="left">
                {dict.settings.timetable.align.left}
              </SelectItem>
              <SelectItem value="center">
                {dict.settings.timetable.align.center}
              </SelectItem>
              <SelectItem value="right">
                {dict.settings.timetable.align.right}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
      ...
      <div className="flex flex-col gap-2">
        <label className="block font-bold mb-2">
          {dict.settings.timetable.display}:
        </label>
