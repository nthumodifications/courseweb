...
import RefineControls, {
  RefineControlFormTypes,
  emptyFilters,
} from "@/components/Courses/RefineControls";
...
const CoursePage: NextPage = () => {
  ...
  const [filtersState, setFiltersState] =
    useLocalStorage<RefineControlFormTypes>("course_filters", emptyFilters);
  ...
  const form = useForm<RefineControlFormTypes>({
    defaultValues: useMemo(() => {
      if (searchParams.size > 0) {
        ...
        let params = queryString.parse(searchParams.toString(), {
          arrayFormat: "index",
          parseNumbers: true,
        });
        //@ts-ignore
      ...
      <Pagination>
        <PaginationContent>
          {currentPage != 1 && (
            <PaginationItem>
              <PaginationPrevious
                onClick={() => searchQuery(filters, headIndex - 30)}
              />
            </PaginationItem>
          )}
          {currentPage > 4 && (
  ...
  const searchQuery = async (
    filters: RefineControlFormTypes,
    index: number = 0,
  ) => {
    ...
    try {
      ...
      if (filters.level.length)
        temp = temp.or(
          filters.level
            .map(
              (level) => `and(course.gte.${level}000,course.lte.${level}999)`,
            )
            .join(","),
        );
      if (filters.language.length)
        temp = temp.or(
          filters.language.map((lang) => `language.eq.${lang}`).join(","),
        );
      if (filters.department.length)
        temp = temp.in("department", filters.department);
      if (filters.className)
        temp = temp.or(
          `compulsory_for.cs.{"${filters.className}"},elective_for.cs.{"${filters.className}"}`,
        );
      if (filters.others.includes("xclass"))
        temp = temp.contains("tags", ["X-Class"]);
      if (filters.others.includes("16_weeks"))
        temp = temp.contains("tags", ["16周"]);
      if (filters.others.includes("extra_selection"))
        temp = temp.not("tags", "cs", '{"不可加簽"}');
      if (filters.firstSpecialization || filters.secondSpecialization) {
        temp = temp.or(
          `first_specialization.cs.{"${filters.firstSpecialization ?? ""}"},second_specialization.cs.{"${filters.secondSpecialization ?? ""}"}`,
        );
      }
      if (filters.venues.length) {
        temp = temp.containedBy("venues", filters.venues);
      }
      if (filters.disciplines.length) {
        temp = temp.overlaps("cross_discipline", filters.disciplines);
      }
      if (filters.gecDimensions.length) {
        temp = temp.in("ge_type", filters.gecDimensions); //TODO: should consider changing name to gec_type
      }
      if (filters.geTarget.length) {
        temp = temp.in("ge_target", filters.geTarget);
      }
      if (filters.timeslots.length) {
        if (filters.timeFilter == TimeFilterType.Within)
          temp = temp.containedBy("time_slots", filters.timeslots);
        else if (filters.timeFilter == TimeFilterType.Includes)
          temp = temp.overlaps("time_slots", filters.timeslots); //Overlap works if only one of their timeslots is selected
        //don't include the timeslot filter if it is empty array
        temp = temp.not("time_slots", "eq", "{}");
      }
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-[auto_320px]">
        <Form {...form}>
          <div
            className="flex flex-col w-full space-y-5 px-2 no-scrollbar scroll-smooth"
            ref={scrollRef}
          >
            <FormField
              control={control}
              ...
              render={({ field }) => (
                <div className="flex flex-row min-h-[44px] items-center rounded-md shadow-md bg-secondary text-secondary-foreground sticky top-0 z-10">
                  ...
                  <input
                    className="bg-transparent outline-none flex-1 text-secondary-foreground"
                    placeholder={dict.course.list.search_placeholder}
                    {...field}
                  />
                  <Fragment>
                    {filters.textSearch.length > 0 && (
                      <Button
                        size="icon"
                        variant={"ghost"}
                        onClick={() => setValue("textSearch", "")}
                      >
            ...
            <div className="relative">
              ...
              {loading && (
                <div className="absolute inset-0 bg-white/60 dark:bg-background/60 z-10"></div>
              )}
              <div className="flex flex-col w-full h-full space-y-4 pb-14">
                <div className="flex flex-row justify-between px-3 py-1 border-b dark:border-neutral-800">
                  <h6 className="text-gray-600 dark:text-neutral-400">
                    {toPrettySemester(filters.semester)}{" "}
                    {dict.course.list.courses}
                  </h6>
                  <h6 className="text-gray-600 dark:text-neutral-400">
                    {dict.course.list.found}: {totalCount}{" "}
                    {dict.course.list.courses}
                  </h6>
                </div>
