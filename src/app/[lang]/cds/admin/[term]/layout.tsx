...
const getTermSubmissionDetails = async (term: string) => {
    ...
    const { data: submission_list = [], count: submissions, error: error1 } = await supabase_server
  ...
  params: { lang: string, term: string }
    ...
    return (
        <div className="flex flex-col h-full w-full px-4">
            <h1 className="text-3xl font-bold py-2">{decodeURI(term)}調查 - 全部回覆</h1>
            <div className='flex flex-row items-center rounded-md bg-gray-100 dark:bg-neutral-800 max-w-md my-2'>
                ...
                <Separator orientation="vertical"/>
                <div className='flex-1 px-1 py-3 flex flex-col items-center'>
                    <DownloadSubmissions submissions={submissionsDetails.submission_list} filename={decodeURI(term)} />
                </div>
            </div>
            <Separator/>
            <div className="w-full flex-1 grid grid-cols-[2fr_3fr] gap-4 divide-x divide-gray-200 overflow-y-hidden">
                ...
                <div className='w-full h-full overflow-y-auto pl-4 py-2'>{submissions}</div>
            </div>
            {children}
        </div>
    )
