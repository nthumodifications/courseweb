    ...
    department: [
      {
        code: "EECS",
        name_en:
          "Interdisplinary Program of College of Electrical Engineering and Computer Science",
        name_zh: "電機資訊學院學士班",
      },
      {
        code: "CS",
  ...
  const searchQuery = async (
    ...
    try {
      ...
      if (filters.department.length)
        temp = temp.in(
          "department",
          filters.department.map(({ code }) => code),
        );
      if (filters.timeslots.length) {
        if (filters.timeFilter == TimeFilterType.Within)
          ...
          temp = temp.overlaps("time_slots", filters.timeslots);
    ...
    return (
      ...
      >
        ...
        >
          ...
          <Divider sx={{ mt: "auto" }} />
          <DialogContent
            sx={{ gap: 2, scrollBehavior: "smooth" }}
            ref={scrollRef}
          >
            <InputControl
              control={control}
            ...
            <Accordion expanded={showFilters}>
              <AccordionDetails>
                <div className="flex flex-row flex-wrap">
                  ...
                  <Accordion
                    sx={{ flex: 1, minWidth: "200px", marginBottom: "1rem" }}
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="mb-3 space-y-1">
                      <Link href={`/${language}/courses/${course.raw_id}`}>
                        <h1 className="font-semibold text-lg text-[#AF7BE4]">
                          {course.department} {course.course}-{course.class}{" "}
                          {course.name_zh} -{" "}
                          {(course.teacher_zh ?? []).join(",")}
                        </h1>
                      </Link>
                      <h3 className="text-sm text-gray-800 dark:text-gray-300 mt-0 break-words">
                        {course.name_en}
                      </h3>
                    </div>
            ...
            <Stack direction="row" justifyContent="center">
              ...
              {currentPage != totalPage && (
                <Button
                  size="icon"
                  variant="ghost"
                  onClick={() => searchQuery(filters, (totalPage - 1) * 30)}
                >
                  <ChevronsRight />
                </Button>
              )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 rounded-sm mt-3">
          ...
          <div className="flex flex-col gap-4 px-4">
            ...
            <ButtonGroup buttonFlex={1}>
              <Button variant="outline" onClick={(e) => setOpen(true)}>
                <Plus /> Add Course
              </Button>
              <Dropdown>
                <MenuButton
