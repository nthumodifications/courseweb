  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div
          className={cn(
            "flex flex-row gap-2 border border-border rounded-md p-2 items-center",
            isCoursesEmpty ? "" : "line-through opacity-30",
          )}
        >
...
export const LoginPage = ({ onClose }: { onClose: () => void }) => {
  ...
  const allowed =
    agreeChecked && studentid.trim().length > 0 && password.trim().length > 0;
  ...
  return (
    <>
      {user ? (
        <WelcomeUserPage onClose={onClose} />
      ) : (
        <div className="flex flex-col gap-8" onKeyDown={handleKeyDown}>
          <div className="flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-2">
                <Label className="">{dict.ccxp.studentid}</Label>
                <Input
                  className="w-full"
                  name="studentid"
                  disabled={loading}
              ...
              <div className="flex flex-col gap-2">
                <Label className="">{dict.ccxp.password}</Label>
                <Input
          ...
          <Button onClick={onSubmit} disabled={!allowed || loading}>
            {loading ? <ButtonSpinner /> : dict.settings.account.signin}
          </Button>
        </div>
      )}
    </>
