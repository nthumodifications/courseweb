...
const RefineControls: FC<{ form: UseFormReturn<RefineControlFormTypes> }> = ({
  form,
}) => {
  ...
  const {
    ...
    queryFn: async () => {
      const { data = [], error } = await supabase
        .from("distinct_first_specialization")
        .select("unique_first_specialization");
      if (error) throw error;
      return data!.map(
        ({ unique_first_specialization }) => unique_first_specialization!,
      );
    },
  });
  const {
    ...
    queryFn: async () => {
      const { data = [], error } = await supabase
        .from("distinct_second_specialization")
        .select("unique_second_specialization");
      if (error) throw error;
      return data!.map(
        ({ unique_second_specialization }) => unique_second_specialization!,
      );
    },
  });
  const {
  ...
  const {
    ...
    queryFn: async () => {
      const { data = [], error } = await supabase
        .from("distinct_cross_discipline")
        .select("discipline");
      if (error) throw error;
  ...
  return (
    <div className="p-4 flex flex-col overflow-auto bg-background">
      ...
      <div className="flex flex-col gap-4 mt-4">
        ...
        <MultiCheckboxControl
          control={form.control}
          name="geTarget"
          options={GETargetCodes.map((code) => ({
            value: code.code,
            label: `${code.code} ${language == "zh" ? code.short_zh : code.short_en}`,
          }))}
        ...
        <AutocompleteShadcn
          ...
          options={classList.map((classname) => ({
            value: classname,
            label: getFormattedClassCode(classname, semester, lang),
          }))}
        ...
        <MultiCheckboxControl
          ...
          options={[
            ...
            {
              value: "extra_selection",
              label: dict.course.refine["extra_selection"],
            },
          ]}
        ...
        <AutocompleteShadcn
          ...
          loading={load1}
          options={firstSpecial.map((special) => ({
            value: special,
            label: special,
          }))}
          label={dict.course.refine.specialization}
        />
        <AutocompleteShadcn
          control={form.control}
          ...
          loading={load2}
          options={secondSpecial.map((special) => ({
            value: special,
            label: special,
          }))}
          label={dict.course.refine.specialization}
