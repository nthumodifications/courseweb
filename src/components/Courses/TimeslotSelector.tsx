...
const TimeslotSelector: FC<{ value: string[], onChange: (newvalue: string[]) => void }> = ({ value = [], onChange }) => {
    ...
    const handleSelectColumn = (day: string) => () => {
        ...
        const columnSelected = scheduleTimeSlots.some(timeSlot => isSelected(day+timeSlot.time));
        if(columnSelected) {
            onChange(value.filter(tc => !scheduleTimeSlots.some(timeSlot => tc == day+timeSlot.time)))
        } else {
            onChange([...value, ...scheduleTimeSlots.map(timeSlot => day+timeSlot.time)])
        }
    }
    ...
    return <div className='grid grid-cols-[repeat(7,24px)] grid-rows-[repeat(14,24px)] gap-1 text-center text-sm'>
        ...
        {days.map(day => (
            <div 
                key={day} 
                className='rounded-md hover:bg-gray-100 dark:hover:bg-neutral-900 cursor-pointer'
        ...
        {scheduleTimeSlots.map(timeSlot => [
            <div 
                key={timeSlot.time} 
                className='rounded-md hover:bg-gray-100 dark:hover:bg-neutral-900 cursor-pointer'
                onClick={handleSelectRow(timeSlot.time)}>{timeSlot.time}
            </div>,
            ...days.map(day => (
                <div 
                    key={day+timeSlot.time}
                    className={`${!isSelected(day+timeSlot.time)?"bg-gray-200 hover:bg-gray-100 dark:bg-neutral-800 dark:hover:bg-neutral-900":"bg-gray-400 hover:bg-gray-300 dark:bg-neutral-600 dark:hover:bg-neutral-700"} transition-colors cursor-pointer`}
