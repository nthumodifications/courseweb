...
export const CommentsItem = ({ comment }: { comment: Awaited<ReturnType<typeof getComments>>[number]; }) => {
    ...
    return <Dialog>
        <DialogTrigger asChild>
            <div className="grid gap-2 py-4 px-2">
                <div className="grid gap-0.5">
                    ...
                    <div className="flex flex-row items-center gap-2 text-sm">
                        <span>涼度: </span><Rating rating={comment.easiness} size={16} variant="yellow" showText={false} disabled/>
                    </div>
                </div>
        ...
        <DialogContent>
            <DialogHeader>
            <DialogTitle>{toPrettySemester(comment.courses!.semester)} {comment.courses?.name_zh} {comment.courses!.teacher_zh.join(',')}</DialogTitle>
            <DialogDescription>
                <div className="grid gap-0.5">
                    <p className="text-sm text-gray-500">{formatDistanceToNow(new Date(comment.posted_on), { locale: zhTW })}發</p>
                    <div className="flex flex-row items-center gap-2 text-sm">
