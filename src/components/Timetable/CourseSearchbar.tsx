...
const CourseSearchbar = ({
  ...
  onAddCourse: (course: CourseDefinition) => void;
  semester?: Semester;
}) => {
  ...
  const filterOptions = createFilterOptions({
    matchFrom: "any",
    stringify: (option: CourseDefinition) =>
      `${option.name_zh} ${option.name_en} ${option.teacher_zh.join(" ")} ${option.teacher_en?.join(" ")} ${option.raw_id}`,
  });
  ...
  return (
    <Autocomplete
      ...
      getOptionLabel={(option) =>
        `${option.department} ${option.course}-${option.class} ${option.name_zh} ${option.name_en}`
      }
      renderOption={(props, option) => (
        <AutocompleteOption {...props}>
          <ListItemContent sx={{ fontSize: "sm" }}>
            <p className="font-semibold">
              {option.department} {option.course}-{option.class}{" "}
              {option.name_zh}
            </p>
            <p className="text-xs">{option.name_en}</p>
            <p className="text-xs text-gray-400">
              {option.teacher_zh.join(",")} {option.teacher_en?.join(",")}{" "}
            </p>
