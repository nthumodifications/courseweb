...
const TimetableSlotHorizontal: FC<TimetableSlotProps> = ({ course, tableDim, fraction = 1, fractionIndex = 1,  ...props }) => {
    ...
    <div 
        ...
        {...props}
        >
        <div className={cn('flex flex-col justify-start items-start text-left h-full p-1 select-none')}>
            <div className='flex-1 flex flex-col justify-start items-start text-left w-full'>
                ...
                {preferences.display.time && <span className='text-xs line-clamp-1' id="time_slot">{scheduleTimeSlots[course.startTime].start} - {scheduleTimeSlots[course.endTime].end}</span>}

            </div>
            {preferences.display.venue && <VenueChip venue={course.venue} color={course.textColor} textColor={course.color} />}
        </div>
    </div>
    );
}
