...
export const getTimeLabel = (time: { hours: number; minutes: number }) => `${String(time.hours).padStart(2, "0")}:${String(time.minutes).padStart(2, "0")}`
...
export function TimeSelect({
    ...
    const [selected_time, setSelected] = React.useState<{ hours: number; minutes: number }>(getHourMinute(date));
    const [inputValue, setInputValue] = React.useState(getTimeLabel(selected_time));

    React.useEffect(() => {
        onDateChange?.(set(date, {...selected_time, seconds: 0, milliseconds: 0}));
    }, [selected_time]);

    React.useEffect(() => {
        //check if selected_time is not same as date
        const time = getHourMinute(date);
        if (selected_time.hours === time.hours && selected_time.minutes === time.minutes) return;
        setSelected(time);
