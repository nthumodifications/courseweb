...
export const addShortLink = async (url: string) => {
    ...
    const res = await fetch(endpoint(key, process.env.CLOUDFLARE_WORKER_ACCOUNT_ID!, process.env.CLOUDFLARE_KV_SHORTLINKS_NAMESPACE!), {
        method: "PUT",
        headers: {
...
export const getShortLink = async (key: string) => {
    const text = await fetch(endpoint(key, process.env.CLOUDFLARE_WORKER_ACCOUNT_ID!, process.env.CLOUDFLARE_KV_SHORTLINKS_NAMESPACE!), {
        method: "GET",
        headers: {
            Authorization: `Bearer ${process.env.CLOUDFLARE_KV_API_TOKEN}`,
