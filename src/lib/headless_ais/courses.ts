...
export const getLatestCourseEnrollment = async (ACIXSTORE: string, dept: string) => {

    const html = await fetch("https://www.ccxp.nthu.edu.tw/ccxp/COURSE/JH/7/7.2/7.2.7/JH727002.php", {
        "headers": {
            "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
            "accept-language": "en-US,en;q=0.9",
            "cache-control": "max-age=0",
    ...
    const hasGeType = Array.from(headerCells).some(cell => (cell.textContent ?? "").includes('通識類別'));
