...
export const signInToCCXP = async (studentid: string, password: string): SignInToCCXPResponse => {
    ...
    try {
        ...
        const classCodeHTML = await fetch(`https://www.ccxp.nthu.edu.tw/ccxp/INQUIRE/TDM/1/1.1/TDM1110.php?ACIXSTORE=${result.ACIXSTORE}`, {
            "headers": {
              "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
              "accept-language": "en-US,en;q=0.9",
              "cache-control": "max-age=0",
              ...
              "sec-fetch-user": "?1",
              "upgrade-insecure-requests": "1",
              "referer": `https://www.ccxp.nthu.edu.tw/ccxp/INQUIRE/IN_INQ_STU.php?ACIXSTORE=${result.ACIXSTORE}`
            },
            keepalive: true,
            "body": null,
            "method": "GET",
            ...
            .then(res => res.arrayBuffer())
            .then(arrayBuffer => new TextDecoder('big5').decode(new Uint8Array(arrayBuffer)))
        const { document: classCodeDoc } = parseHTML(classCodeHTML, 'text/html');

        const targetElement = classCodeDoc.querySelector('td[height="30"] font[color="#0180FE"]');
        ...
        const data = {
            ...
            classCode: {
                ...
                degreeType: degreeType === '' ? "unknown" : degreeType,
