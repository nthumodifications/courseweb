...
const app = new Hono()
  ...
  .post(
    ...
    async (c) => {
      const { user_id, refreshToken } = c.req.valid("form");
  ...
  .post(
    ...
    async (c) => {
      const { authToken, deviceId, session_id } = c.req.valid("form");
  ...
  .post(
    ...
    async (c) => {
      ...
      parcelCards.forEach((card) => {
        ...
        const name =
          card.querySelector(".parcel-name")?.textContent?.trim() ?? "";
        const logistic =
