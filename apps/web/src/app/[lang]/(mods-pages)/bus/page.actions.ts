...
export const getBusesSchedules = async (
  bus_type: "all" | "main" | "nanda",
  day: "all" | "weekday" | "weekend" | "current",
  direction: "up" | "down",
) => {
  const res = await fetch(
    `https://api.nthusa.tw/buses/schedules/?bus_type=${bus_type}&day=${day}&direction=${direction}`,
  );
  ...
  if (!res.ok) {
    ...
    const data = await fetch(
      `https://nthumods.com/fallback_data/bus/${bus_type}_${day}_${direction}.json`,
    );
