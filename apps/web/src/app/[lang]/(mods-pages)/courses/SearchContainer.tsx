...
import { PageHeader } from "@courseweb/ui";
...
export function InfiniteHits(props: Parameters<typeof useInfiniteHits>[0]) {
  ...
  return (
    <div
      ref={scrollRef}
      className={cn(
        "relative min-w-0 overflow-x-hidden overflow-y-auto",
        status !== "idle" ? "opacity-80" : "",
      )}
      style={{ height: "calc(100vh - 200px)" }}
    >
      <div className="min-w-0 space-y-1 pb-20">
        {/* Regular items */}
        {hits.map((hit) => (
...
const SearchContainer = memo(
  ({
    ...
    return (
      <div className="flex min-w-0 w-full gap-6">
        <aside className="hidden md:flex min-w-0 w-72 shrink-0 flex-col gap-3">
          <PageHeader
            title={dict.course.refine.title}
            actions={<ResetFiltersButton />}
          />
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-4">
          ...
          <div className="min-w-0">
            <PageHeader
              title={dict.course.refine.search_results}
              description={
                <span className="flex min-w-0 items-center gap-2 text-xs tabular-nums text-muted-foreground">
                  <span className="truncate">
                    {nbHits} {dict.course.refine.results} ({processingTimeMS}ms)
                  </span>
                  <span aria-hidden="true">·</span>
                  <a
                    aria-label={dict.course.search.by_algolia}
                    title={dict.course.search.by_algolia}
                    href="https://www.algolia.com/?utm_medium=AOS-referral"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex min-w-0 max-w-32 shrink-0 items-center text-muted-foreground hover:text-foreground"
                  >
                    <svg viewBox="0 0 572 64" className="h-4 w-auto max-w-full">
                      <path
                        fill={"#36395A"}
