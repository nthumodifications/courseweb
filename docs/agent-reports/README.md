...
2. **Recruitment** — apply `services/api/migrations/20260910_120000_create_recruitment.sql`, then confirm the private `resumes` bucket exists with the restrictive policy. Confirm the `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` Worker secrets.
